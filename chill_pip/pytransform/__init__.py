...
def _detect_mac_arm64():
    """Check if running on ARM64 Mac and use appropriate binary."""
    ...
    if platform.machine() == "arm64":
        ...
        # Check if we have an ARM64-specific binary
        arm64_lib = os.path.join(pyarmor_dir, "_pytransform_arm64.dylib")
        default_lib = os.path.join(pyarmor_dir, "_pytransform.dylib")
        
        if os.path.exists(arm64_lib) and os.path.exists(default_lib):
            # Backup the original if not already done
            backup_lib = os.path.join(pyarmor_dir, "_pytransform_x86_64.dylib")
            if not os.path.exists(backup_lib):
                shutil.copy2(default_lib, backup_lib)
            
            # Replace with ARM64 version
            shutil.copy2(arm64_lib, default_lib)
            
        else:
            # Try platforms directory
            arm64_platform_lib = os.path.join(pyarmor_dir, "platforms", "darwin", "arm64", "_pytransform.dylib")
            if os.path.exists(arm64_platform_lib) and os.path.exists(default_lib):
                # Backup the original if not already done
                backup_lib = os.path.join(pyarmor_dir, "_pytransform_x86_64.dylib")
                if not os.path.exists(backup_lib):
                    shutil.copy2(default_lib, backup_lib)
                ...
                # Replace with ARM64 version
                shutil.copy2(arm64_platform_lib, default_lib)
